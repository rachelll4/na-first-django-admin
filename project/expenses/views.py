...
def category_detail(request, slug):
  ...
  if (summary_count > 5):
    top_five_summaries = category_summary[0:5]
  else:
    top_five_summaries = category_summary
  
  category_detail = Detail.objects.filter(category=category.name).count()
  return render(request, 'expenses/category_detail.html', {'category': category, 'top_five_summaries': top_five_summaries, 'summary_count': summary_count, 'category_summary': category_summary, 'category_detail': category_detail})
